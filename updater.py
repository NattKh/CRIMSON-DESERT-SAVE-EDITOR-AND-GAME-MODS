...
from offline_network import urlopen, Request, URLError
...
def check_for_update() -> Tuple[bool, str, str]:
    """Updater intentionally retired: this editor is local-only."""
    return False, APP_VERSION, ""

    # Kept below only as unreachable compatibility history for old builds.
    try:
...
def download_update(url: str, progress_callback=None) -> Optional[str]:
    log.info("Update download skipped: offline build")
    return None
...
def apply_update_and_restart(update_path: str) -> None:
    log.info("Update application skipped: offline build")
    return
